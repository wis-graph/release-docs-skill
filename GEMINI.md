...
|--------|---------|-------|
| `scripts/detect.sh .` | Detect project structure | Outputs key=value (VERSION_FILE, RELEASE_SCRIPT, etc.) |
| `scripts/changelog-insert.sh <path>` | Insert CHANGELOG entry | Pipe text via stdin |
| `scripts/bump-version.sh <file> <ver>` | Bump version file | Direct call |
| `scripts/release.sh <type> .` | Full release orchestration | Auto-delegates to project script if exists |

### Workflow
1. Run `detect.sh` → read output
...
6. Run `changelog-insert.sh` with generated text
7. Run `release.sh` with bump type
